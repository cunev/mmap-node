...
class CustomArgs_t {
public:
  void push_back(const void *ptr, size_t size) { append(ptr, size); }

  template <typename T> void push_back(const T *ptr) { append(ptr, sizeof(T)); }

  template <typename T> void push_back(const std::basic_string<T> &str) {
    append(str.data(), str.size() * sizeof(T));
  }

  template <class T> void push_back(const std::vector<T> &cVec) {
    append(cVec.data(), cVec.size() * sizeof(T));
  }

  template <class T, size_t N> void push_back(const std::array<T, N> &arr) {
    append(arr.data(), arr.size() * sizeof(T));
  }

#if _MSC_VER >= 1900
  template <typename... Args> void push_back(const std::tuple<Args...> &tpl) {
...
enum eLoadFlags {
  ...
  RebaseProcess = 0x40, // If target image is an .exe file, process base address
                        // will be replaced with mapped module value
...
struct LoadData {
  ...
  LoadData(MappingType mtype_, enum LdrRefFlags ldrFlags_)
      : mtype(mtype_), ldrFlags(ldrFlags_) {}
};

// Image mapping callback
enum CallbackType {
  PreCallback, // Called before loading. Loading type is decided here
  PostCallback // Called after manual mapping, but before entry point
               // invocation. Loader flags are decided here
};

using MapCallback = LoadData (*)(CallbackType type, void *context,
                                 Process &process, const ModuleData &modInfo);
...
public:
  ...
  BLACKBONE_API static bool MapImage(HANDLE handle, size_t size, void *buffer);
  ...
  /// <param name="mapCallback">Mapping callback. Triggers for each mapped
  /// module</param> <param name="context">User-supplied callback
  /// context</param> <returns>Mapped image info </returns>
  BLACKBONE_API call_result_t<ModuleDataPtr>
  MapImage(const std::wstring &path, eLoadFlags flags = NoFlags,
           MapCallback mapCallback = nullptr, void *context = nullptr,
  ...
  /// <param name="asImage">If set to true - buffer has image memory
  /// layout</param> <param name="flags">Image mapping flags</param> <param
  /// name="mapCallback">Mapping callback. Triggers for each mapped
  /// module</param> <param name="context">User-supplied callback
  /// context</param> <returns>Mapped image info</returns>
  BLACKBONE_API call_result_t<ModuleDataPtr>
  MapImage(size_t size, void *buffer, bool asImage = false,
           eLoadFlags flags = NoFlags, MapCallback mapCallback = nullptr,
           void *context = nullptr, CustomArgs_t *pCustomArgs_t = nullptr);
  ...
  /// Reset local data
  /// </summary>
  BLACKBONE_API inline void reset() {
    _images.clear();
    _pAContext.Reset();
    _usedBlocks.clear();
  }
...
private:
  ...
  call_result_t<ModuleDataPtr> MapImageInternal(
      ...
      eLoadFlags flags = NoFlags, MapCallback ldrCallback = nullptr,
      void *ldrContext = nullptr, CustomArgs_t *pCustomArgs_t = nullptr);
  ...
  call_result_t<ModuleDataPtr> FindOrMapModule(const std::wstring &path,
  ...
  call_result_t<uint64_t>
  RunModuleInitializers(ImageContextPtr pImage, DWORD dwReason,
                        CustomArgs_t *pCustomArgs_t = nullptr);
  ...
  /// <returns></returns>
  call_result_t<ModuleDataPtr> FindOrMapDependency(ImageContextPtr pImage,
                                                   std::wstring &path);
  ...
  /// <param name="asImage">if true - 'path' points to a valid PE file,
  /// otherwise - 'path' points to separate manifest file</param> <returns>true
  /// on success</returns>
...
private:
  ...
  MapCallback _mapCallback =
      nullptr; // Loader callback for adding image into loader lists
