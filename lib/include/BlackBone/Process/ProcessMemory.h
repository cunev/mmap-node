...
public:
  ...
  /// <param name="desired">false if caller will be responsible for block
  /// deallocation</param> <returns>Memory block. If failed - returned block
  /// will be invalid</returns>
  BLACKBONE_API call_result_t<MemBlock>
  Allocate(size_t size, DWORD protection = PAGE_EXECUTE_READWRITE,
           ptr_t desired = 0, bool own = true);
  ...
  /// will be invalid</returns>
  BLACKBONE_API call_result_t<MemBlock>
  AllocateClosest(size_t size, DWORD protection = PAGE_EXECUTE_READWRITE,
                  ptr_t desired = 0, bool own = true);
  ...
  BLACKBONE_API NTSTATUS Free(ptr_t pAddr, size_t size = 0,
  ...
  /// <returns>Status</returns>
  BLACKBONE_API NTSTATUS Protect(ptr_t pAddr, size_t size, DWORD flProtect,
                                 DWORD *pOld = NULL);
  ...
  /// If true, function will try to read all committed pages in range ignoring
  /// uncommitted ones. Otherwise function will fail if there is at least one
  /// non-committed page in region.
  /// </param>
  /// <returns>Status</returns>
  BLACKBONE_API NTSTATUS Read(ptr_t dwAddress, size_t dwSize, PVOID pResult,
  ...
  /// </param>
  /// <returns>Status</returns>
  BLACKBONE_API NTSTATUS Read(const std::vector<ptr_t> &adrList, size_t dwSize,
                              PVOID pResult, bool handleHoles = false);
  ...
  /// <returns>Status</returns>
  BLACKBONE_API NTSTATUS Write(ptr_t pAddress, size_t dwSize,
                               const void *pData);
  ...
  /// <param name="pData">Buffer to write</param>
  /// <returns>Status</returns>
  BLACKBONE_API NTSTATUS Write(const std::vector<ptr_t> &adrList, size_t dwSize,
                               const void *pData);
  ...
  template <class T> inline call_result_t<T> Read(ptr_t dwAddress) {
  ...
  template <class T> inline NTSTATUS Read(ptr_t dwAddress, T &result) {
  ...
  inline NTSTATUS Read(std::vector<ptr_t> &&adrList, T &result) {
    return Read(std::forward<std::vector<ptr_t>>(adrList), sizeof(result),
                &result);
  };
  ...
  /// <returns>Status</returns>
  template <class T> inline NTSTATUS Write(ptr_t dwAddress, const T &data) {
  ...
  /// </summary>
  /// <param name="includeFree">If true - non-allocated regions will be included
  /// in list</param> <returns>Found regions</returns>
  BLACKBONE_API std::vector<MEMORY_BASIC_INFORMATION64>
  EnumRegions(bool includeFree = false);
  ...
  BLACKBONE_API void protectionCasting(MemProtectionCasting casting) {
    _casting = casting;
  }
